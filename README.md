...
・ 以下のビルトインコマンドを実装してください  
	1.echo （-n オプションの挙動を含む）  
 	1.cd　（相対パス、または絶対パスのみで動作する）  
 	1.pwd　（オプションの処理を含まない）  
   	1.export　（オプションの処理を含まない）  
   	1.unset　（オプションの処理を含まない）  
   	1.env　（オプションと引数の処理を含まない）  
   	1.exit　（オプションの処理を含まない）  
・ 「'」は、すべてのシーケンス文字の解釈を抑制します  
・ 「"」 は、「$」をのぞくすべてのシーケンス文字の解釈を抑制します  
・ リダイレクト  
	1.「<」で、入力のリダイレクトを行ってください  
 	1.「>」で、出力のリダイレクトを行ってください  
 	1.「<<」では、現在の入力元から区切り文字列だけを含む行が現れるまで読み込みます。履歴の更新は必要ありません！  
	1.「>>」は、出力のリダイレクトを追記モードで行ってください  
...
・ インタラクティブのとき（訳者註: コマンド入力待ちの状態で）：  
	1.Ctrl - C : 次の行に、プロンプトを表示してください  
	1.Ctrl - D : シェルを終了してください  
	1.Ctrl - \ : 何も行わないでください  
